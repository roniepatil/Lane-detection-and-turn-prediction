...
out = cv2.VideoWriter('prob1_output//histogramequalization.mp4',cv2.VideoWriter_fourcc(*'mp4v'), 10, size)
...
for i in range(len(img_array)):
    output = perform_equalization_and_merge_channels(img_array[i])
    cv2.imwrite('prob1_output/histogram_equalization'+ str(i) +'.png',output)
    out.write(output)
out.release()